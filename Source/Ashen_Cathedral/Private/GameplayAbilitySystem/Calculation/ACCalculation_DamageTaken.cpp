...
void UACCalculation_DamageTaken::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	// - 패링 성공: 피격 데미지 0, 방어자 체간 누적 없음, 공격자(Source)에게 체간 역공(BasePostureDamage * 1.5) + 짧은 경직(Stagger) 전송
	// - 블록 성공: 피격 데미지 90% 감소, 방어자 본인에게 체간 데미지(BasePostureDamage * 0.8) 누적
	// 공격의 Blockable/Parryable 여부는 EffectSpec의 DynamicAssetTags(UACAnimNotify_IncomingAttackWarning이
	// UACAbility_Attack::CurrentAttackDefenseTags를 거쳐 주입)로 판정한다.
	// - Parry 성공 조건: IsSuccessfulParry (ParryWindow 상태 && IsAttackParryable && 정면 방어 각도)
	// - Block 성공 조건: IsSuccessfulBlock (Blocking 상태 && IsAttackBlockable && 정면 방어 각도)
	// 두 판정 모두 이벤트 발송/HitReact 억제/Hit Cue 억제와 공통이며, 어느 쪽도 성립하지 않으면 일반 피격 처리한다.
	const FGameplayTagContainer& AttackTags = EffectSpec.GetDynamicAssetTags();

	// 공격자(Source) 정보 — 패링 역공/Stagger 적용과 Block 성공 판정(정면 각도)에 공통으로 사용한다.
	const UAbilitySystemComponent* SourceASCConst = ExecutionParams.GetSourceAbilitySystemComponent();
	AActor* SourceActor = SourceASCConst ? SourceASCConst->GetAvatarActor() : nullptr;
	...
	{
		if (UACFunctionLibrary::IsSuccessfulParry(SourceActor, TargetASC->GetAvatarActor(), AttackTags))
		{
			FinalDamageDone = 0.f;
			...
			if (BasePostureDamage > 0.f && SourceASC)
			{
			...
			// 패링당한 공격자에게 짧은 경직(Stagger)을 부여해 재공격을 잠깐 락아웃한다.
			if (SourceASC && StaggerEffectClass)
			{
				const FGameplayEffectSpecHandle StaggerSpecHandle = SourceASC->MakeOutgoingSpec(StaggerEffectClass, 1.f, SourceASC->MakeEffectContext());
				if (StaggerSpecHandle.IsValid())
				{
					const float StaggerDuration = FMath::FRandRange(StaggerDurationMin, StaggerDurationMax);
					UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(StaggerSpecHandle, ACGameplayTags::Shared_SetByCaller_StaggerDuration, StaggerDuration);
					SourceASC->ApplyGameplayEffectSpecToSelf(*StaggerSpecHandle.Data.Get());
				}
			}

			// Boss(Enemy)의 GA_BossParry가 대기 중인 WaitGameplayEvent를 깨운다. 리스너가 없으면 no-op이라
			// Player Parry(GA_Block 내부 판정) 시에도 안전하게 호출할 수 있다.
			if (AActor* TargetActor = const_cast<AActor*>(TargetASC->GetAvatarActor()))
			{
				FGameplayEventData ParrySuccessPayload;
				ParrySuccessPayload.EventTag = ACGameplayTags::Enemy_Event_ParrySuccess;
				ParrySuccessPayload.Instigator = SourceActor;
				ParrySuccessPayload.Target = TargetActor;
				UAbilitySystemBlueprintLibrary::SendGameplayEventToActor(TargetActor, ACGameplayTags::Enemy_Event_ParrySuccess, ParrySuccessPayload);
			}
		}
		else if (UACFunctionLibrary::IsSuccessfulBlock(SourceActor, TargetASC->GetAvatarActor(), AttackTags))
		{
