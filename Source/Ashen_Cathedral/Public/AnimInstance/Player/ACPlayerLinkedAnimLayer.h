 ...
 *
 * 이 클래스는 주 플레이어 애님 인스턴스('UACPlayerAnimInstance') 위에 동적으로 연결(링크)되어 무기 장착 상태(예: 칼 장착)에 따른 고유한 상체 애니메이션 (Idle, 걷기, 공격 등)을 덮어쓰는 데 사용
 * 'UACAnimInstanceBase'를 상속받으며, 'UACPlayerAnimInstance'의 변수(예: 속도, 방향)에 접근하여 기본 Locomotion 상태와 동기화된 애니메이션을 재생합니다.
 * @see UACPlayerAnimInstance
 * @see UACAnimInstanceBase
 */
UCLASS()
