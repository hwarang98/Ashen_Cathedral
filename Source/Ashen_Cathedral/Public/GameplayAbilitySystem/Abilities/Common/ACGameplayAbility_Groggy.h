...
/**
 * 플레이어·적 공용 체간 붕괴(그로기) 어빌리티.
 *
 * AttributeSet이 Posture >= MaxPosture를 감지하면
 * Shared.Event.PostureBrokenTriggered 이벤트를 발사하고,
 * 이 어빌리티가 트리거되어 체간 붕괴 상태를 처리한다.
 *
 * 어빌리티 활성 중에는 ActivationOwnedTags로 Shared_Status_PostureBroken이 자동 부여되며,
