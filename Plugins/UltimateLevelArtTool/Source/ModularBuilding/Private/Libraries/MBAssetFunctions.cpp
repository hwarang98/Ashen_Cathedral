...
FAssetData UMBAssetFunctions::GetAssetDataFromObject(const UObject* Object)
{
	const FString assetPath = GetAssetPathFromObject(Object);
	return GetAssetDataFromPath(assetPath);
}

FString UMBAssetFunctions::GetAssetPathFromObject(const UObject* AssetObject)
{
	FString path;
...
FAssetData UMBAssetFunctions::GetAssetDataFromPath(const FString& AssetPath)
{
...
UTexture2D* UMBAssetFunctions::GenerateThumbnailForAsset(const FAssetData& InAssetPath,const EMBAssetThumbnailFormant InImageFormat)
{
	FString PackageFilename;
	const FName ObjectFullName = FName(*InAssetPath.GetFullName());
	TSet<FName> ObjectFullNames;
	ObjectFullNames.Add(ObjectFullName);
	const EImageFormat Format = InImageFormat == EMBAssetThumbnailFormant::JPEG ? EImageFormat::JPEG : EImageFormat::PNG;
